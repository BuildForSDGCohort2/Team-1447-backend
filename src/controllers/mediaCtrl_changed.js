                ...
                if (result.rowCount > 0) {
                    ...
                    res.status(201).json({
                        ...
                        data: {
                            ...
                            createdOn: result.rows[0].media_date_of_pub,
                    ...
                    //     data: {
                    //         mediaId : result.rows[0].media_id,
                    //         createdOn: result.rows[0].media_posted_on,
                    //         caption: result.rows[0].media_caption,
                    //         mediaUrl: result.rows[0].media_url
                    //     }
                    // }
     ...
     */
        ...
        static async getOneMedia(req, res, next) {
            try {
                ...
                // const query = `SELECT media_url, media_caption, media_id, media_posted_by, media_date_of_pub, 
                //                comment_id, comment, comment_date_of_pub, comment_posted_by, media_posted_on,
                //                user_name, user_id FROM media LEFT JOIN comments ON media_id=media_posted_on 
                //                LEFT JOIN users ON comment_posted_by=user_id WHERE media_id=${mediaId} ORDER BY media_date_of_pub DESC;`
                ...
                if (result.rowCount > 0) {
                    ...
                    const mod = await pool.query('SELECT * FROM COMMENTS WHERE MEDIA_POSTED_ON=$1', [mediaId]);
                    if (mod.rowCount > 0){
                        result.rows.comments = mod.rows;
                    }else{
